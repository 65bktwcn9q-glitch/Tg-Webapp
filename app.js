...
const adminResetLimits = document.getElementById("admin-reset-limits");
const adminToggleAds = document.getElementById("admin-toggle-ads");
const adminRefreshMetrics = document.getElementById("admin-refresh-metrics");
...
const setAdminStatus = (text) => {
  ...
  adminStatus.textContent = `Статус: ${text}`;
};
...
if (adminResetLimits) {
  adminResetLimits.addEventListener("click", () => {
    setAdminStatus("сброс лимитов...");
    fetchJson(api.adminAction, {
      method: "POST",
      ...
      setAdminStatus(response?.status ?? "лимиты сброшены");
      showSheet("Админ: лимиты", response?.message ?? "Лимиты сброшены для всех пользователей.");
    });
  });
}

if (adminToggleAds) {
  adminToggleAds.addEventListener("click", () => {
    setAdminStatus("переключение рекламы...");
    fetchJson(api.adminAction, {
      method: "POST",
      body: JSON.stringify({ action: "toggleAdsGlobal" }),
    }).then((response) => {
      if (response?.summary) {
        updateSummary(response.summary);
      }
      setAdminStatus(response?.status ?? "реклама обновлена");
      showSheet("Админ: реклама", response?.message ?? "Состояние рекламы обновлено.");
    });
  });
}

if (adminRefreshMetrics) {
  adminRefreshMetrics.addEventListener("click", () => {
    setAdminStatus("обновление метрик...");
    fetchJson(api.adminSummary).then((response) => {
      if (!response) {
        setAdminStatus("метрики недоступны");
        showSheet("Админ: метрики", "Не удалось получить метрики. Попробуйте позже.");
        return;
      }
      setAdminStatus("метрики обновлены");
      showSheet(
        "Админ: метрики",
        `Retention: ${response.retention}\nVIP конверсия: ${response.vipConversion}\nАктивные пользователи: ${response.activeUsers}\nУроков сегодня: ${response.lessonsToday}`
      );
